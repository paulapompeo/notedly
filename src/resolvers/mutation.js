...
// This will generate a Gravatar image URL from a user’s email address.
...
module.exports = {
  ...
  toggleFavorite: async (parent, { id }, { models, user }) => {
    // if no user context is passed, thrown auth error
    if (!user) {
    ...
    const hasUser = noteCheck.favoritedBy.indexOf(user.id); // return the index, or -1 if not found

    // if the user exists in the list
    // pull them from the list and reduce the favouriteCoung by 1
    if (hasUser >= 0) {
